...
**Session-2 checkpoints** (the QLD election tweet workflow, one workspace archive per phase) — these are workshop-specific. Download from **sih.tools/wordflow → Materials → Session 2 checkpoint files**:
...
- `Checkpoint_D.zip` — after Topic Modelling on two corpora + Add to Workspace into per-gender child blocks.
