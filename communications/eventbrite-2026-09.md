# Eventbrite copy: three posts, 29-30 September 2026

Three separate Eventbrite events, one per session. Each post opens with the same condensed series paragraph (so every landing page explains the bundle), then its own session blurb. Fill the "Also in this series" links once all three events exist.

Eventbrite setup reminders:

- Add the unticked opt-in checkbox at registration ("I'd like to hear about future workshops from this team") per the participant-data policy; registration alone is not consent.
- Summary field: Eventbrite caps it at 140 characters; the summaries below fit.
- Venue for all three: The Quadrangle (A14), Western Tower Boardroom (room S495), University of Sydney, Camperdown NSW 2006.
- Capacity, organiser profile, and ticket names are Chao's call; nothing below assumes them.

## Shared series paragraph (opens every description)

This session is part of a bundle of three free in-person workshops for researchers, held 29-30 September at the University of Sydney by the Language Data Commons of Australia (LDaCA), the Reusable and Accessible Public Interest Documents (RAPID-CDL) project, and the Sydney Informatics Hub, co-hosted by the Sydney Corpus Lab and the Centre for Computational Social Science. Each session works as a standalone workshop: register for any one, or come to all three, as the Wednesday sessions build on each other especially well. Open to researchers from all disciplines and universities, and HDR students are especially welcome. Please bring your own laptop for these interactive sessions.

---

## Post 1: Making the most of your qualitative transcripts with a little bit of computing

**Date:** Tuesday 29 September 2026, 14:00-15:30

**Summary (Eventbrite short field):**
A pathway through 'too much' interview data with a little computational help, without giving up the qualitative foundation of your work.

**Description:**

[Shared series paragraph]

Qualitative research projects sometimes get away from us: we end up with more interview material than we know what to do with. This workshop gives you a pathway for navigating 'too much' with a little computational assistance, without giving up on the core qualitative foundation of your work.

Working through a case study with demonstration tools from the Language Data Commons of Australia, you will:

- Spot the consistency issues in transcripts that confuse a computer (and yourself and your collaborators)
- Transform example transcripts into a more consistent, structured format
- See how computers can help you search and find things when you have 'too much'
- See computational methods that surface the 'big picture' and what you might be missing, while you keep close-reading your transcripts

**Also in this series:** [link to Post 2], [link to Post 3]

## Post 2: Getting Started with the Proceedings of Federal Parliament in your Research

**Date:** Wednesday 30 September 2026, 09:15-10:45 (please arrive at 9:00 for a 9:15 start)

**Summary (Eventbrite short field):**
125 years of Federal Parliament debates are on the record. Learn to find what your research needs with ParlInfo and RAPID-CDL tools.

**Description:**

[Shared series paragraph]

The Proceedings of Australian Federal Parliament record every debate held since Federation in 1901. Whether you're interested in language, politics, society or history, there's likely something relevant to your research in the 15,000 transcribed sessions, if you can find it!

This workshop shows you how to get started. We'll cover the necessary context of how Parliament works through its official transcripts and the ParlInfo search interface, then show how the datasets and tools being created by the RAPID-CDL research infrastructure project (https://rapid-cdl.edu.au) make it even easier to get what you need for your research.

**Also in this series:** [link to Post 1], [link to Post 3, flagged as the natural follow-on]

*Branding note (not for the post): no LDaCA logo on this event's imagery; ARDC / Community Data Lab branding applies.*

## Post 3: Analysing parliamentary discourse at scale with the new Wordflow app

**Date:** Wednesday 30 September 2026, 11:00-12:30

**Summary (Eventbrite short field):**
Hands-on with LDaCA Wordflow, a free point-and-click text analysis app, on thousands of parliamentary debates. No coding needed.

**Description:**

[Shared series paragraph]

You've found the debates you need. What can you do with several thousand of them at once?

This hands-on workshop introduces LDaCA Wordflow, a free point-and-click app for analysing text, built for researchers who don't write code. Using the Proceedings of Federal Parliament as our example, we'll explore what becomes possible once a collection is searchable and structured: tracing how language shifts across parliaments and decades, reading individual speeches in context, and comparing how different speakers and periods treat the same issue.

Bring your laptop and work along with us. No coding experience needed. This session follows straight on from the morning's Getting Started workshop and pairs especially well with it, though it also stands alone.

**Also in this series:** [link to Post 1], [link to Post 2, flagged as the recommended morning session]
