...
> **Rewritten 2026-08-27** to the current design: the jobs theme-coding task on the 226-tweet block, Title-case `Promise` / `Cuts` / `Other` codebook, a preview → compare → correct loop and an examples run, all measured against a **human-verified reference annotation**, a **shared workshop key**, checkpoint workspace archives a–e as rescues (**a** = the Tweets block everyone builds first; **d** = the v2 codebook block; **e** = the showcase codebooks), published as a GitHub release: https://github.com/milysun/wordflow-workshop/releases/tag/online-2026-08-28. **Prompts are not saved in checkpoints or codebook blocks**: after any load, participants must paste the v1/v2 prompt again; brief the helpers, the responsible-AI slide before the tool, and the showcase segment. Wall-clock rows and every chat snippet: `facilitator/run-of-show-online.html`. Participant steps: `participant/hands-on-annotation-online.md` (its § numbers are used below).
...
**Setup before start** (full list in `pre-workshop-checklist-online.md`): recording **OFF** and auto-record disabled; checkpoint ZIPs a–e + `tweets_job_reference.csv` published on the release page (links prefilled in the panel's fields; the 'ALL download links' snippet is for helpers); the shared key created at 12:35 with a spend cap, tested (model list loads, one Preview succeeds), pasted into the panel; room model `google/gemini-2.5-flash-lite`, fallback model in the panel; your own Wordflow on a clean workspace, multi-tab ON, cursor kit on; full MBP screen shared.
...
## 15:19 · Demo: the fourth lever, a stronger model; then the hands-on ends (2 min)

Say it plainly: *"the hands-on is over; watch from here."* Same v2 codebook and prompt, model switched to **GLM 5.3 flash**; start Run All and talk over it (it's slow), or show the rehearsal result: v1 0.809 → v2 **0.867**, against flash-lite's 0.805. Land: *"a stronger, newer model is the fourth lever: higher cost, slower, no guarantee, and only worth anything once the codebook is right."*

## 15:21 · After coding: the coded block is data (slide 9, 3 min; the key point)

Slide 9, unhurried: *"coding was never the goal. A good codebook turned 226 tweets into a variable, your angle on the data, and every other tool can read it."* If you have 60 seconds, ONE live move: **Preprocessing → Filter `job.AI_v2` = `Cuts` → Add to Workspace → Trends grouped by `party`**. Otherwise talk the four cards: filter to a corpus per code; Frequency comparing Promise vs Cuts vocabularies; Trends of codes over the campaign by party; Concordance and Topic Modelling inside one code. Land: *"the annotated block carries the answers to your question; carry it into the tools that give you the targeted view."* Then the provenance line: codebook, prompt, model, κ travel with the block.

## 15:24 · Showcase, in one breath (demo only, 1 min; first cut if late)

One codebook only if time, else just name them: **A** sentiment toward the LNP (aspect, not sentence), **B** place outside Queensland, **C** more than two people; all three ready to run in **Checkpoint e**, prompts in the sheet's appendix. *"Classic NLP problems, now one codebook away; the checking workflow you just did is what makes them trustworthy."*

## 15:25 · Take it home + close on time (slide 10, 5 min)

Slide 10, top to bottom, briskly: the shared key is gone at 3:30, so real research means your own key or a **local model** (Add Provider → Custom: Ollama, LM Studio); **ethics approval** decides models and providers; keep the loop and document prompt, codebook, model, κ; **no AI required**: a team of human coders each in their own column, Compare To gives percent agreement, Cohen's κ or Krippendorff's α across multiple reference columns; Wordflow is under active development: **Feedback** button, one-click updates, cite via the sidebar's quote icon. Thank the helpers. Follow-up email: Session 1 recording, materials, checkpoints. **Close at 15:30.**
...
| Way behind at 15:00 | Cut in order: showcase → GLM demo → v2 run (point at Checkpoint d + appendix for home) → examples run. Run All, the 'coded block is data' slide, and take-home are the non-negotiables. |
