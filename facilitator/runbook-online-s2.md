...
## 15:19 · Demo: the fourth lever, a stronger model; then the hands-on ends (3 min)

Say it plainly: *"the hands-on is over; watch from here."* Same v2 codebook and prompt, model switched to **GLM 5.3 flash**; start Run All and talk over it (it's slow), or show the rehearsal result: v1 0.809 → v2 **0.867**, against flash-lite's 0.805. Land: *"a stronger, newer model is the fourth lever: higher cost, slower, no guarantee, and only worth anything once the codebook is right."* Then the showcase, **one codebook only**: **A** sentiment toward the LNP (aspect, not sentence), Preview, from the copy bank; name **B** (place outside Queensland) and **C** (more than two people) in one breath and point at **Checkpoint e**, which holds all three ready to run at home. Land: *"classic NLP problems, now one codebook away; the checking workflow you just did is what makes them trustworthy."* First cut if late.
