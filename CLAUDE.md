...
- **Right-click is reserved for in-tool visualisation shortcuts, NOT for block/column management.** Specifically: **Frequency cloud/list — right-click a word to add it to stopwords** (it hides instantly). There may be other in-tool right-click actions in future tools. **Block-level actions (Rename, Clone, Tokenise, Detach, Undo, Redo, Delete) come from a menu icon on the data block itself.** The same icon shape appears at the end of each column header for column-level actions. **Never write "right-click → Rename / Delete / ..." for block or column management** — use "open the block's menu icon" or "open the column menu" instead. The Frequency stopword shortcut IS an exception worth documenting in materials.
- **"Add to Workspace" is the button name.** Each tool's results panel (Concordance, Frequency comparative, Trends selection, Topic Modelling detach, Preprocessing operations) has an **`Add to Workspace`** button that turns the current result into a new data block. Don't write "add as data block" / "add as block" / "add as a new block" — write *"click Add to Workspace"* (the button name) and, if needed, *"…which creates a new data block"* for the description.
